...
fn input_ids_safe_to_forward(value: &serde_json::Value) -> bool {
    ...
    for key in [
        ...
        "reasoning_effort",
        "task",
        // The three below steer the Kimi-K3 encoder specifically. Unlike the
        ...
        // `response_format`) or as the effort preamble (`thinking_effort`, which
        // is a DIFFERENT field from the `reasoning_effort` above). Whether the
        // engine injects a byte-identical block is unverifiable today, so these
        // ids stay router-side.
        "response_format",
        "tool_choice",
        "thinking_effort",
    ] {
